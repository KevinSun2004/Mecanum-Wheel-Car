...
void PS2_Decode()
{
    if (PS2_RawData[2] == 0x5A) {
        // Key
        PS2_Data.Key_Select  = (~PS2_RawData[3] >> 0) & 0x01;
        PS2_Data.Key_Start   = (~PS2_RawData[3] >> 3) & 0x01;
        ...
        // Zero Zone
        if (abs(PS2_Data.Rocker_LX) < ZEROZONE) PS2_Data.Rocker_LX = 0;
        if (abs(PS2_Data.Rocker_LY) < ZEROZONE) PS2_Data.Rocker_LY = 0;
        if (abs(PS2_Data.Rocker_RX) < ZEROZONE) PS2_Data.Rocker_RX = 0;
        if (abs(PS2_Data.Rocker_RY) < ZEROZONE) PS2_Data.Rocker_RY = 0;
    }
}
...
void PS2_Read_Data()
{
    ...
    PS2_RawData[0] = PS2_Rx_and_Tx(0x01);
    PS2_RawData[1] = PS2_Rx_and_Tx(0x42);
    for (int i = 2; i < 9; i++)
